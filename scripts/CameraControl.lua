...
function events.RENDER(delta, context)
	if context == "FIRST_PERSON" or context == "RENDER" or (not client.isHudEnabled() and context ~= "MINECRAFT_GUI") then
		...
		if pose.stand or pose.crouch then
			...
			posOffset  = posOffset - vec(0, (pose.sleep or pose.elytra) and 0 or 14, pose.sleep and -24 or 8) / 16
			nameOffset = posOffset - vec(0, (pose.sleep and -16 or pose.elytra and 0 or -2) * modelEyeHeight, pose.sleep and -2 or -24) / 16
		...
		local cameraPos = pokemonParts.Body:partToWorldMatrix():apply() + vec(0, 0.2, 0) + client:getCameraDir() * 0.1
